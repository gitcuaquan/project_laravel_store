            ...
            <div class="col-xl-5 p-5 mt-4 bg-b_2">
                <form action="{{ route('cat.create') }}" method="post">
                    ...
                    <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                        {!! Form::label('name', 'Tên Danh Mục Con') !!}
                        {!! Form::text('name', null, ['class' => 'form-control', 'required' => 'required']) !!}
                        <small class="text-danger">{{ $errors->first('name') }}</small>
                    </div>
                    <label for="cat">
                        Chọn danh mục phục thuộc
                    </label><br>
                    @error('parent_id')
                        <span class="text-danger">( {{ $message }} )</span>
                    @enderror
                    <select name="parent_id" class="form-select" id="cat">
                    ...
                    <div class="">
                        ...
                        <a href="{{ route('cat.add_parent') }}" class="btn mt-5  btn-primary">Thêm Danh Mục Cha</a>
                    </div>
                </form>
